...
class InterfaceConnection extends EventEmitter {
    ...
    retryConnect(self) {
        self.log('info', "Check retry: ", self.interfaceName)
        if (self.failureCount > self.maxFailureCount && self.wasConnected == false) {
            self.log('info', "Giving up on ", self.interfaceName)
            clearInterval(self.retryConnectTimer);
            self.retrying = false;
            return
        }
        if (self.connected == false) {
            self.log('info', "Reconecting to ", self.interfaceName)
            self.failureCount += 1;
    ...
    initInterface() {
        ...
        this.rpcClient.methodCall('init', [self.getInitUrl(), 'homey_' + self.interfaceName], function (err, res) {
            if (err) {
                self.log('info', "Failed to connect:", self.interfaceName, err)
                ...
                self.log('info', "Connected to", self.interfaceName)
                self.failureCount = 0;
                self.connected = true;
    ...
    checkConnection() {
        ...
        if (elapsed > pingTimeout) {
            self.log('info', 'ping timeout', this.interfaceName, elapsed);
            self.initInterface();
            return;
        }
        if (elapsed >= (pingTimeout / 2)) {
            self.log('info', "Sending ping to ", this.interfaceName)
