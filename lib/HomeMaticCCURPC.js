...
class HomeMaticCCURPC extends EventEmitter {
    constructor(logger, homey, type, serial, ccuIP) {
        ...
        this.ccuIP = ccuIP;
        this.type = type;
        this.transport = Constants.TRANSPORT_RPC;
        this.serial = serial;
    ...
    createConnections() {
        ...
        Object.keys(this.interfaceConfigs).forEach((interfaceName) => {
            self.connections[interfaceName] = new Connection(this.logger, interfaceName, self.interfaceConfigs[interfaceName])
            self.connections[interfaceName].on('event', (event) => {
...
module.exports = HomeMaticCCURPC;