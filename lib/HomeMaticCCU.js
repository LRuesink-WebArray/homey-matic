...
class HomeMaticCCU extends EventEmitter {
    constructor(type, serial, ccuIP) {
        ...
        this.MQTTClient.on('connect', function() {
            self.MQTTClient.subscribe('homey/#', function(err) {
                if (err) {
                    console.log(err);
                } else {
    ...
    onMessage(topic, message) {
        ...
        console.log(topic, jsonMsg);
        let eventName = "event-" + jsonMsg.iface + "-" +jsonMsg.channel + "-" + jsonMsg.datapoint;
        console.log(eventName, jsonMsg.value);
        this.emit(eventName, jsonMsg.value);
    ...
    getValue(interfaceName, address, key) {
        ...
        return new Promise(function (resolve, reject) {
            fetch('http://192.168.1.85/addons/red/get_value?channel='+address+'&datapoint='+key+'&iface='+interfaceName)
             .then(res => res.json())
             .then(body => {
                 console.log(interfaceName + '-' + address + ': ' + body)
                resolve(body)})
